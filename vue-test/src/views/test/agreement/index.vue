<template>
	<view class="page">
		<!-- Region 顶部导航栏 -->
		<u-navbar title="合同记录" :placeholder="true" bgColor="#2989FF" :border="false"
			autoBack
			leftIconColor="#fff"
		...
		<view class="container">
			<view class="search u-border-bottom">
				<u-search v-model="listParams.searchValue" placeholder="搜索合同或客户或商机名称" actionText="搜索"
					@custom="search" 
					@clear="searchClear"
				/>
			</view>
			...
			<view class="list">
				<view class="list-item" v-for="item in list" :key="item.id" >
					...
					<view class="info" @tap="() => toDetails(item.id)">
						...
						<view class="info-item">
							<text>商机名称</text>
							<text>{{ item.opportunitiesName || '' }}</text>
						</view>
						<view class="info-item">
							<text>对接人员</text>
...
<script>
	import { getMpCustomContractPage } from '@/common/api/customer.js';
		...
		methods: {
			...
			async getList({
				...
				try{
					...
					let { rows: data, total } = await getMpCustomContractPage(params);
					total ??= 0;
					data ??= [];
					this.list.push(...data);
