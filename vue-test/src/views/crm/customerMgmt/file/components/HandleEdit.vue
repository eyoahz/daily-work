    ...
    >
      <Skeleton active :loading="loading">
        <BasicForm @register="registerForm">
          ...
          <!--              <a-button v-if="type == 'add'" type="primary" @click="() => openCustomerModal(true)">-->
          <!--                选择客户-->
          <!--              </a-button>-->
          <!--            </div>-->
          <!--          </template>-->
...
<script lang="ts" setup>
  ...
  import { getDetails, insert, update } from '/@/api/customerMgmt/file';
  ...
  const [registerDrawer, { setDrawerProps, closeDrawer }] = useDrawerInner(async (data) => {
    ...
    switch (unref(data?.type)) {
      case 'add': {
        ...
        setFieldsValue(unref(data?.data ?? {}));
        break;
      }
      case 'edit': {
        getTitle.value = '编辑';
